-- Load with loadfile for multiple buffers.
-- no longer binds the primary gpu by default. must call bindGPU(address)
-- Modified version of 'doubleBuffering' by Igor Timofeev
...
local function formattedText(x, y, data)
	if y >= drawLimitY1 and y <= drawLimitY2 then
		...
		while charIndex <= unicodeLen(data) do
			if x >= drawLimitX1 and x <= drawLimitX2 then
...
------------------------------------------------------------------------------------------------------
-- dont auto bind the primary gpu
