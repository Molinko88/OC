--[[
	Edited by Molinko a.k.a FastEddie

	# Changes:
		- Removed 'fs' dependency. Not used.
		- Removed global dependency 'advancedLua'
		+ Added 'readUnicodeChar', 'tableSize', 'numberToFixedSizeByteArray', 'byteArrayToNumber' from 'advancedLua'
]]
...
local function numberToFixedSizeByteArray(number, size)
...
local function readNumberFromFile(file, countOfBytes)
	...
	return byteArrayToNumber(byteArray)
end
...
encodingMethods.save[6] = function(file, picture)
	...
	file:write(string.char(tableSize(groupedPicture)))
	...
	for alpha in pairs(groupedPicture) do
		...
		writeByteArrayToFile(file, numberToFixedSizeByteArray(tableSize(groupedPicture[alpha]), 2))
		...
		for symbol in pairs(groupedPicture[alpha]) do
			...
			file:write(string.char(tableSize(groupedPicture[alpha][symbol])))
			...
			for background in pairs(groupedPicture[alpha][symbol]) do
				...
				file:write(string.char(tableSize(groupedPicture[alpha][symbol][background])))
				...
				for foreground in pairs(groupedPicture[alpha][symbol][background]) do
					...
					file:write(string.char(tableSize(groupedPicture[alpha][symbol][background][foreground])))
